...
PATTERN is a single element or comma separated list."""
)
@click.option(
    "--apikey",
    required=True,
...
@click.option(
    "--dry", help="Dry run flag", is_flag=True,
)
def updateContentFiltering(apikey, orgid, patterns, action, intent, dry):
    ...
    for net in networks:
        ...
        if "appliance" in net.get(
            ...
            click.echo(
                click.style(
                    f"🚫\t NO SECURITY APPLIANCES\t\t{netId}\t NAME:\t{netName}",
                    fg="yellow",
                )
            )
